...
func NewCountService() *gatt.Service {
	...
	s.AddCharacteristic(gatt.MustParseUUID("11fac9e0-c111-11e3-9246-0002a5d5c51b")).HandleRead(
		gatt.ReadHandlerFunc(func(rsp gatt.ResponseWriter, req *gatt.Request) {
			fmt.Fprintf(rsp, "count: %d", n)
			n++
		}))

	s.AddCharacteristic(gatt.MustParseUUID("16fe0d80-c111-11e3-b8c8-0002a5d5c51b")).HandleWrite(
		gatt.WriteHandlerFunc(func(r gatt.Request, data []byte) (status byte) {
			log.Println("Wrote:", string(data))
			return gatt.StatusSuccess
		}))

	s.AddCharacteristic(gatt.MustParseUUID("1c927b50-c116-11e3-8a33-0800200c9a66")).HandleNotify(
